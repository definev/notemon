...
import 'package:gottask/components/digit.dart';
...
class SlideCountdownClockState extends State<SlideCountdownClock> {
  ...
  void _init() {
    ...
    initStream = Stream<DateTime>.periodic(Duration(milliseconds: 1000), (_) {
      if (widget.onPause == false) {
        if (timeLeft.inSeconds == 0) {
          if (widget.onDone != null) widget.onDone();
          widget.isPlay = false;
        } else if (widget.onChanged != null || widget.isPlay == true) {
          timeLeft -= Duration(seconds: 1);
          widget.onChanged(timeLeft);
        }
      } else {
        if (widget.onPaused != null) widget.onPaused();
      }
      return time;
    }).takeWhile((element) => widget.isPlay);
    timeStream = initStream.asBroadcastStream();
  }
